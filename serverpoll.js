...
//                 io.to(socket.id).emit('showNextQuestionButton');
//             }
//         }
...
//     //     // If the first player is not "tcshost", they will also be a host
//     //     if (rooms[roomName].players.length === 1) {
...
// const HOST = '10.33.0.21'; // Change from '192.168.29.153' to '0.0.0.0'
...
io.on('connection', (socket) => {
    ...
    function startTimer(roomName, duration) {
        ...
        let timeLeft = duration; // Set the local timeLeft
        rooms[roomName].votingAllowed = true; // Allow voting initially
        rooms[roomName].timerInterval = setInterval(() => {
            if (timeLeft <= 0) {
                clearInterval(rooms[roomName].timerInterval);
    ...
    socket.on('playAgain', (roomName) => {
        if (rooms[roomName].hosts.includes(socket.id)) {
            ...
            rooms[roomName].currentQuestionIndex = 0;
            rooms[roomName].votes = [];
            rooms[roomName].quizQuestions = []; // Clear previous questions
            rooms[roomName].isAvailable = true; // Make the room available again
            io.to(roomName).emit('resetQuiz'); // Notify all users to reset their view
        }
    });
    ...
    //     // Notify all users in the room to redirect
    //     io.to(roomName).emit('redirect', { url: '../trial/index.html' });
    // });

    socket.on('endPoll', (roomName) => {
        if (rooms[roomName]) {
            if (rooms[roomName].timerInterval) {
                clearInterval(rooms[roomName].timerInterval); // Clear the timer if it exists
            }
            delete rooms[roomName]; // Delete the room
            io.to(roomName).emit('redirect', { url: '../slides/index.html' }); // Notify all users in the room to redirect
        }
    });
